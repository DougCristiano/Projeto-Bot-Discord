...
module.exports = {
	...
	async execute(interaction) {
		try {
			...
			if (channelFilter) {
				const lowerChannel = channelFilter.toLowerCase().trim()
				filteredLogs = logs.filter((log) => {
					// Verifica entradas e saídas
					if (log.canal && log.canal.toLowerCase().trim() === lowerChannel) {
						return true
					}
					// Verifica mudanças de canal (origem ou destino)
					if (log.tipo === 'mudanca') {
						const origemMatch = log.canalOrigem?.toLowerCase().trim() === lowerChannel
						const destinoMatch = log.canalDestino?.toLowerCase().trim() === lowerChannel
				...
				if (filteredLogs.length === 0) {
					return await interaction.editReply(
						`❌ Não foram encontradas atividades para o canal "${channelFilter}" na data ${dateStr}.`
					)
				}
			}

			// Gera o conteúdo formatado para enviar
			const formattedContent = filteredLogs
				.map((log) => {
					const emoji = log.tipo === 'entrada' ? '🟢' : log.tipo === 'saida' ? '🔴' : '🔄'
					const nome = log.apelido || log.usuario

					if (log.tipo === 'mudanca') {
						return `[${log.timestamp}] ${emoji} ${nome} mudou de ${log.canalOrigem} para ${log.canalDestino}`
					} else {
						const acao = log.tipo === 'entrada' ? 'entrou no canal' : 'saiu do canal'
						return `[${log.timestamp}] ${emoji} ${nome} ${acao}: ${log.canal}`
					}
				})
				.join('\n')
		...
		try {
			...
			if (focusedOption.name === 'canal') {
				...
				if (filtered.length === 0 && focusedValue === '') {
					const allChannels = voiceChannels.slice(0, 25).map((name) => ({
						name: name,
						value: name,
					}))
